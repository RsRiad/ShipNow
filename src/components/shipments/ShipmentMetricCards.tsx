...
const fallbackMetrics: MetricCardData[] = [
  {
    ...
    subtext: "this week",
    isPositive: true,
    icon: <Truck className="w-5 h-5 text-[#856DF3] stroke-[2]" />,
  },
  {
    id: "pending",
    title: "Pending",
    ...
    subtext: "this week",
    isPositive: true,
    icon: <Clock className="w-5 h-5 text-[#856DF3] stroke-[2]" />,
  },
  {
    id: "delivery",
    title: "Delivery",
  ...
  },
  {
    ...
    subtext: "this week",
    isPositive: true,
    icon: <CheckSquare className="w-5 h-5 text-[#856DF3] stroke-[2]" />,
  },
];
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-0">
      {cards.map((card) => (
        <div
          key={card.id}
          className="bg-white border border-[#E5E5E7] rounded-[14px] sm:rounded-[16px] p-3.5 sm:p-5 shadow-2xs hover:shadow-sm transition-all duration-200 flex flex-col justify-between"
        >
          {/* Top Row: Soft Rounded Icon + Title & Menu */}
          <div className="flex items-center justify-between mb-2.5 sm:mb-4">
            <div className="flex items-center gap-2 sm:gap-2.5">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-[10px] sm:rounded-[12px] bg-[#ECE6FF] flex items-center justify-center shrink-0">
                {card.icon}
              </div>
              <span className="text-[12px] sm:text-[15px] font-medium text-[#64748B] truncate max-w-[90px] sm:max-w-none">
                {card.title}
              </span>
            </div>
            <button
              type="button"
              className="hidden sm:block text-[#94A3B8] hover:text-[#475569] transition p-1 cursor-pointer shrink-0"
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-1.5 sm:gap-2 mt-1">
            <span className="text-[24px] sm:text-[34px] font-bold text-[#1E293B] tracking-tight leading-none">
              {card.count}
            </span>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              <div
                className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center shrink-0 ${
                  card.isPositive ? "bg-[#E6F4EA]" : "bg-[#F3E8FF]"
                }`}
              >
                {card.isPositive ? (
                  <ChevronUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#137333] stroke-[2.5]" />
                ) : (
                  <ChevronDown className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#7E22CE] stroke-[2.5]" />
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:gap-1 lg:flex-col lg:items-start leading-tight whitespace-nowrap">
                <div className="flex items-center gap-1">
                  <span className="text-[#64748B] text-[10px] sm:text-[11px] font-normal">
                    {card.prefixText}
                  </span>
                  <span
                ...
                <span className="text-[#94A3B8] text-[9px] sm:text-[11px] font-normal">
                  {card.subtext}
                </span>
              </div>
            </div>
