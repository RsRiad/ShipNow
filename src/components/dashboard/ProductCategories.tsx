  ...
  return (
    <div className="w-full bg-[#FEFEFE] p-4 rounded-[12px] border border-[#F0F0F2] flex flex-col gap-3.5 h-full">
      ...
      <div className="flex items-center justify-between">
        <span className="font-normal text-[14px] leading-[18px] text-[#757575]">
          Total Products
        </span>
        <span className="font-bold text-[22px] leading-[24px] text-[#333333]">
          1,000
        </span>
      </div>

      {/* Horizontal Bar Chart */}
      <div className="flex gap-1 rounded-[8px] overflow-hidden h-[36px]">
        {categories.map((cat) => (
          <div
            key={cat.name}
            className="h-full"
        ...
        {categories.map((cat) => (
          <div key={cat.name} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-semibold text-[12px] leading-[16px] text-[#333333]">
                {cat.name}
              </span>
            </div>

            {/* Count + Percentage */}
            <div className="flex items-center gap-2 bg-[#F0F0F0] rounded-[4px] px-1 py-0.5">
              <span className="font-normal text-[11px] leading-[14px] text-[#757575]">
                {cat.count} products
              </span>
              <div className="w-px h-3.5 bg-[#E0E0E0]" />
              <span className="font-bold text-[11px] leading-[14px] text-[#333333]">
                {cat.percentage}%
              </span>
            </div>
          </div>
